routerAdd(
  'POST',
  '/backend/v1/trigger-contact-alerts',
  (e) => {
    let alertDays = 30
    try {
    ...
    for (let contact of contacts) {
      ...
      try {
        const lastInteraction = $app.findFirstRecordByFilter(
        ...
        for (let admin of admins) {
          ...
          try {
            $app.findFirstRecordByFilter(
              'notifications',
              "user = {:user} && action_payload = {:payload} && action_type = 'contact_alert' && read = false",
              '',
              {
                user: admin.id,
                payload: contact.id,
          ...
          notif.set(
            'message',
            `O contato favorito ${contact.getString('name')} não possui interações há mais de ${alertDays} dias. Retome o contato.`,
          )
    ...
    return e.json(200, { created: createdCount })
  },
  $apis.requireAuth(),
)
