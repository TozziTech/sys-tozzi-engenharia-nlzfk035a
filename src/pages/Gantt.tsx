...
const MOCK_PROJECTS: Project[] = [
  ...
  {
    id: '3',
    name: 'Instalações Shopping',
    client: 'Iguatemi',
    discipline: 'Elétrico',
    status: 'Atrasado',
    startDate: fmt(subDays(today, 40)),
    endDate: fmt(subDays(today, 5)),
    progress: 40,
    engineer: 'Ana Paula',
  },
  {
...
export default function Gantt() {
  ...
  const { allDays, months, chartStart } = useMemo(() => {
    const dates = filtered.flatMap((p) => [parseISO(p.startDate), parseISO(p.endDate)])
  ...
  return (
    <div className="flex flex-col h-[calc(100vh-4rem)] p-4 md:p-6 gap-6 w-full max-w-full overflow-hidden">
      ...
      <div className="flex flex-wrap items-center gap-3 bg-card p-3 rounded-xl border shadow-sm">
        <Select value={disc} onValueChange={setDisc}>
          ...
          <SelectContent>
            ...
            {Array.from(new Set(MOCK_PROJECTS.map((p) => p.discipline))).map((d) => (
        ...
        <Select value={stat} onValueChange={setStat}>
          ...
          <SelectContent>
            ...
            {Array.from(new Set(MOCK_PROJECTS.map((p) => p.status))).map((s) => (
        ...
        <Select value={eng} onValueChange={setEng}>
          ...
          <SelectContent>
            ...
            {Array.from(new Set(MOCK_PROJECTS.map((p) => p.engineer))).map((e) => (
        ...
        <div className="flex min-w-max w-full">
          ...
          <div className="w-72 flex-shrink-0 sticky left-0 z-30 bg-card border-r shadow-[4px_0_12px_rgba(0,0,0,0.03)]">
            ...
            {filtered.map((p) => (
              <div
                key={p.id}
                className="h-14 border-b flex flex-col justify-center px-4 bg-card group hover:bg-muted/50 transition-colors"
              >
          ...
          <div className="relative flex-1" style={{ width: totalWidth, minWidth: totalWidth }}>
            ...
            <div className="flex flex-col w-full relative">
              {filtered.map((p) => {
                const sDay = differenceInDays(parseISO(p.startDate), chartStart)
                const dur = differenceInDays(parseISO(p.endDate), parseISO(p.startDate)) + 1
                return (
                  <div
                    key={p.id}
                  ...
                  >
                    <div
                      ...
                      title={`${p.name} - ${p.status} - ${p.progress}%`}
                    >
